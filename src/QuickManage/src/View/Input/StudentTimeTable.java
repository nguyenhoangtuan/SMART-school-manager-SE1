...
import View.CustomComponent.CellStudentTimeTable;
 ...
 * @author Lyn
 */
public class StudentTimeTable extends JDialog implements Ultility{
    ...
    public StudentTimeTable(Student s) {
        ...
        motherPanel.setBorder(BorderFactory.createTitledBorder(resourse.getString("TIME_TABLE_STUDENT_T_TXT")));
    ...
    public void addCellToTable() {
        cells = new CellStudentTimeTable[6][rows];
        for (int y = 0; y < rows; y++) {
            for (int x = 0; x < 6; x++) {
                cells[x][y] = new CellStudentTimeTable(x, y, this);
    ...
    public void setMap(String code) {
        ...
        for (; part2 < part3; part2 = part2 + 0.25) {
            ...
            if (model.isPaid(current)) {
                cells[part1 - 2][convertHoursToNumber(part2)].setStatus("Paid");
            } else {
                cells[part1 - 2][convertHoursToNumber(part2)].setStatus("Unpaid");
            }
        }
